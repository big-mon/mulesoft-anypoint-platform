...
async def test_get_access_token_fetches_and_caches(monkeypatch):
    ...
    http_client = FakeHTTPClient(
        post_responder=lambda url, **kwargs: {"access_token": "token-1"}
    )
...
@pytest.mark.asyncio
async def test_get_access_token_is_safe_under_concurrency(monkeypatch):
    """It performs a single token fetch for concurrent callers."""
    monkeypatch.setenv("ANYPOINT_CLIENT_ID", "client-id")
    monkeypatch.setenv("ANYPOINT_CLIENT_SECRET", "client-secret")
    monkeypatch.setenv("ANYPOINT_ORGANIZATION_ID", "org-id")
    monkeypatch.setenv("ANYPOINT_BASE_URL", "https://example.com")

    async def post_responder(url, **kwargs):
        await asyncio.sleep(0)
        return {"access_token": "token-1"}

    http_client = FakeHTTPClient(post_responder=post_responder)
    client = AuthClient(http_client, config=Config())
